...
class Maze:
    def __init__(self, filepath):
        ...
        for Nodes in self.raw_data: 
            correspond_node = list(filter(lambda x: str(x) != 'nan', Nodes[1:5]))
        ...
        #BFS_best所需
        self.all_dis = np.zeros((13,13), dtype = int)
        ...
        for nodes in self.raw_data:
            ...
            for i in range(1,5):    # NORTH = 1,EAST = 2,SOUTH = 3,WEST  = 4
                if str(nodes[i]) != "nan":
                    if i == 1:
                        temp.Successors.append((int(nodes[i]),1,int(nodes[i+4])))
                    elif i == 2:
                        temp.Successors.append((int(nodes[i]),3,int(nodes[i+4])))
                    elif i == 3:
                        temp.Successors.append((int(nodes[i]),4,int(nodes[i+4])))             
                    elif i == 4:
                        temp.Successors.append((int(nodes[i]),2,int(nodes[i+4])))
    ...
    def BFS_best(self):
        for i in range(13):
            self.all_dis[i][i] = 255
            for j in range(i+1,13):
                self.BFS_2(self.deadend[i],self.deadend[j])
                self.all_dis[i][j] = len(self.getActions_2())-1
                self.all_dis[j][i] = len(self.getActions_2())-1
            self.all_dis[0][i] = 255
        #print(self.all_dis)
        return 0
...
if __name__ == '__main__':
    test = Maze("data/big_maze_111.csv")
    '''
    test.BFS_2(1,48)
